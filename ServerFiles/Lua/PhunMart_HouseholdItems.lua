return {{
    abstract = true,
    key = "base-household",
    tab = "Household",
    tags = "household",
    inventory = {
        min = 1,
        max = 5
    },
    price = {
        currency = {
            base = 1,
            min = 3,
            max = 5
        }
    }
}, {
    name = "Base.Umbrella",
    inherits = "base-household"
}, {
    name = "Base.UmbrellaBlack",
    inherits = "base-household"
}, {
    name = "Base.UmbrellaRed",
    inherits = "base-household"
}, {
    name = "Base.Eraser",
    inherits = "base-household"
}, {
    name = "Base.Plunger",
    inherits = "base-household"
}, {
    name = "Base.Bleach",
    inherits = "base-household"
}, {
    name = "Base.Broom",
    inherits = "base-household"
}, {
    name = "Base.AlarmClock",
    inherits = "base-household"
}, {
    name = "Base.ClosedUmbrellaWhite",
    inherits = "base-household"
}, {
    name = "Base.ClosedUmbrellaRed",
    inherits = "base-household"
}, {
    name = "Base.Lamp",
    inherits = "base-household"
}, {
    name = "Base.LetterOpener",
    inherits = "base-household"
}, {
    name = "Base.DishCloth",
    inherits = "base-household"
}, {
    name = "Base.Sponge",
    inherits = "base-household"
}, {
    name = "Base.UmbrellaWhite",
    inherits = "base-household"
}, {
    name = "Base.RedPen",
    inherits = "base-household"
}, {
    name = "Base.Pencil",
    inherits = "base-household"
}, {
    name = "Base.Soap",
    inherits = "base-household"
}, {
    name = "Base.BluePen",
    inherits = "base-household"
}, {
    name = "Base.ClosedUmbrellaBlack",
    inherits = "base-household"
}, {
    name = "Base.AlarmClock2",
    inherits = "base-household"
}, {
    name = "Base.Scissors",
    inherits = "base-household"
}, {
    name = "Base.Pen",
    inherits = "base-household"
}, {
    name = "Base.Soap2",
    inherits = "base-household"
}, {
    name = "Base.ClosedUmbrellaBlue",
    inherits = "base-household"
}, {
    name = "Base.CleaningLiquid",
    inherits = "base-household"
}, {
    name = "Base.BathTowel",
    inherits = "base-household"
}, {
    name = "Base.UmbrellaBlue",
    inherits = "base-household"
}, {
    name = "Base.Extinguisher",
    inherits = "base-household"
}, {
    name = "Base.RippedSheets",
    inherits = "base-household"
}, {
    name = "Base.ChairLeg",
    inherits = "base-household"
}, {
    name = "Base.CleaningLiquid2",
    inherits = "base-household"
}, {
    name = "Base.WineInGlass",
    inherits = "base-household"
}, {
    name = "Base.Mop",
    inherits = "base-household"
}, {
    name = "FunctionalAppliances.FAPaperTowel",
    inherits = "base-household",
    mod = "FunctionalAppliances"
}, {
    name = "FunctionalAppliances.FAPaperNapkins",
    inherits = "base-household",
    mod = "FunctionalAppliances"
}, {
    name = "SOMW.FireplacePoker",
    inherits = "base-household",
    mod = "SimpleOverhaulMeleeWeapons"
}, {
    name = "SOMW.WalkingCane",
    inherits = "base-household",
    mod = "SimpleOverhaulMeleeWeapons"
}, {
    name = "Base.Cologne",
    inherits = "base-medical",
    mod = "EssentialCrafting"
}, {
    name = "Base.Perfume",
    inherits = "base-medical",
    mod = "EssentialCrafting"
}}
