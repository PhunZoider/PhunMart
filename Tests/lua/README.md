...
rather than about saving. The key still has to be a string: the legacy import
reads a converted `.json`, and JSON keys are only ever strings, so a number on
the Lua side would quietly fail to match anything coming in.
