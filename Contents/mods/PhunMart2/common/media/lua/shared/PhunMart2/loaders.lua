...
local function getCoreShops()
    ...
    local results = {}
    local order = 0
    local all = Core.tools.merge(shopsData, Core.extended or {})
...
local function getModifications()
    ...
    if not isClient() then
        ...
        local data = Core.tools.loadTable(Core.consts.shopsLuaFile)
        if not data then
...
function Core:getShops()
    ...
    local results = Core.tools.merge(modified or {}, core or {})
    return results
end
