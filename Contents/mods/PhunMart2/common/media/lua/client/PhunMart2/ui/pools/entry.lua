...
function UI:createChildren()
    ...
    local lbl = ISLabel:new(pad, pad, FONT_HGT_MEDIUM,
        "Pool references - each entry selects a pool to draw items from.",
        1, 1, 1, 0.7, UIFont.Small, true)
    ...
    list.onMouseUp = function(l, x, y)
        ...
        if row and row > 0 then l.selected = row end
    end
    ...
    btnAdd:initialise(); btnAdd:instantiate()
    if btnAdd.enableAcceptColor then btnAdd:enableAcceptColor() end
    ...
    btnEdit:initialise(); btnEdit:instantiate()
    ...
    btnRemove:initialise(); btnRemove:instantiate()
    if btnRemove.enableCancelColor then btnRemove:enableCancelColor() end
...
function UI:setData(data)
    ...
    for _, ref in ipairs(data.keys or {}) do
        table.insert(self.keys, { key = ref.key or "", weight = ref.weight or 1.0 })
    end
    self:refreshList()
...
function UI:getData()
    ...
    for _, ref in ipairs(self.keys) do
        if ref.key and ref.key ~= "" then
            table.insert(refs, { key = ref.key, weight = tonumber(ref.weight) or 1.0 })
        end
    end
    return { keys = refs }
end
...
function UI:drawRow(y, item, alt)
    ...
    self:setStencilRect(0, math.max(0, y + self:getYScroll()),
        col2x, math.min(self.height, y + self:getYScroll() + self.itemheight))
...
function UI:onAdd()
    self:openKeyWeightDialog(nil, nil, function(key, weight)
        table.insert(self.keys, { key = key, weight = weight })
        self:refreshList()
...
function UI:onEditSelected()
    local list = self.controls.list
    if not (list.selected and list.selected > 0 and list.items[list.selected]) then return end
    ...
    self:openKeyWeightDialog(ref.key, ref.weight, function(key, weight)
        self.keys[idx] = { key = key, weight = weight }
        self:refreshList()
        self.isDirtyValue = true
    end)
end
...
function UI:onMoveUp()
    ...
    if not (i and i > 1) then return end
...
function UI:onMoveDown()
    local list = self.controls.list
    local i = list.selected
    if not (i and i > 0 and i < #self.keys) then return end
    self.keys[i], self.keys[i + 1] = self.keys[i + 1], self.keys[i]
    self:refreshList()
    list.selected = i + 1
    self.isDirtyValue = true
...
function UI:openKeyWeightDialog(existingKey, existingWeight, cb)
    ...
    lblKey:initialise(); lblKey:instantiate(); dlg:addChild(lblKey)

    local txtKey = ISTextEntryBox:new(existingKey or "", pad + 80, pad, dw - pad * 2 - 80, FONT_HGT_SMALL + 4)
    txtKey:initialise(); txtKey:instantiate(); dlg:addChild(txtKey)

    local lblW = ISLabel:new(pad, pad * 2 + FONT_HGT_SMALL + 8, FONT_HGT_SMALL, "Weight:", 1, 1, 1, 1, UIFont.Small, true)
    lblW:initialise(); lblW:instantiate(); dlg:addChild(lblW)

    local txtW = ISTextEntryBox:new(tostring(existingWeight or 1.0), pad + 80, lblW.y, 100, FONT_HGT_SMALL + 4)
    txtW:initialise(); txtW:instantiate(); dlg:addChild(txtW)

    local hint = ISLabel:new(pad, lblW.y + FONT_HGT_SMALL + pad + 4, FONT_HGT_SMALL,
        "Weight: relative draw probability, e.g. 1.0 or 0.5",
        0.65, 0.65, 0.65, 1, UIFont.Small, true)
    hint:initialise(); hint:instantiate(); dlg:addChild(hint)
