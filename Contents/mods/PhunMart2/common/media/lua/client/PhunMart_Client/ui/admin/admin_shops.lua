...
local function copySet(set)
    if not set then return { keys = {} } end
    local copy = { keys = {} }
...
local function copyPoolSets(poolSets)
    if not poolSets then return {} end
    local result = {}
...
local function createSetEditForm(setData, isNew, cb)
    ...
    local titleText = isNew and getText("IGUI_PhunMart_Title_AddPoolSet") or
                          getText("IGUI_PhunMart_Title_EditPoolSet")
    ...
    local form = FormPanel:new({
        ...
        onApply = function(f)
            ...
            if rollMin and rollMax then
                result.roll = { mode = "weighted", count = { min = rollMin, max = rollMax } }
            end
...
local function createEditModal(shopKey, shopDef, cb)
    ...
    local form = FormPanel:new({
        ...
        onApply = function(f)
            ...
            if prob then result.probability = prob end

            local dist = f:getFieldNumber("minDistance")
            if dist then result.minDistance = dist end

            local restock = f:getFieldNumber("restockFrequency")
            if restock then result.restockFrequency = restock end

            local view = f:getFieldValue("defaultView")
            if view == "list" then result.defaultView = "list" end

            local bg = f:getFieldValue("background")
            if bg and bg ~= "" then result.background = bg end

            local sprites = parseCSV(f:getFieldValue("sprites"))
            if sprites then result.sprites = sprites end

            local unpSprites = parseCSV(f:getFieldValue("unpSprites"))
            if unpSprites then result.unpoweredSprites = unpSprites end
            ...
            if cb then cb(shopKey, result) end
            f:close()
        end,
    })
    ...
    form:addListField("poolSets", getText("IGUI_PhunMart_Lbl_PoolSets"), {
        ...
        rows = 3,
        columns = {
            { name = getText("IGUI_PhunMart_Col_Pool"), size = 0 },
            { name = getText("IGUI_PhunMart_Col_Price"), size = 0.55 },
            { name = getText("IGUI_PhunMart_Col_Roll"), size = 0.80 },
        },
